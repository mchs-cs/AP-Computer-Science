...
public class BactorDisplay extends AbstractDisplay {
	private Class<?> cl;
	private String imageFilename;
	private static final String imageExtension = ".gif";
	private Map<String, Image> tintedVersions = new HashMap<String, Image>();
	private boolean basicBattleBug=false;

	public BactorDisplay() throws IOException {
		super();
		this.cl=this.getClass();
		this.imageFilename="battle/pack/Bactor";
		//		imageFilename = cl.getName().replace('.', '/');
		System.out.println("BactorDisplay(No Param Constructor)imageFilename=" + this.imageFilename);
	...
	public BactorDisplay(Class<?> cl) throws IOException
	{
		this.cl = cl;
		imageFilename = cl.getName().replace('.', '/');
		System.out.println("BactorDisplay(Class<?> cl Constructor)imageFilename=" + this.imageFilename);
		URL url = cl.getClassLoader().getResource(imageFilename + imageExtension);
	...
	{
		System.out.println("BactorDisplay:Sub-Draw->" + obj.toString());
...
//			System.out.println("BactorDisplay:Draw->" + imageSuffix);
			if (!imageSuffix.equals("_BasicBattleBug")) {
				tinted=untinted;
			}

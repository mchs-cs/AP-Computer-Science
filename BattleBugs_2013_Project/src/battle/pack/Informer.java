...
public class Informer 
{
	...
	public ArrayList<Objective> getObjectiveListBugs()
	{
		ArrayList<Objective> objectives = new ArrayList<Objective>();
		ArrayList<Bactor> bactors = new ArrayList<Bactor>();
		ArrayList<Location> locs = new ArrayList<Location>();
		Grid<Actor> gr = this.bactor.getGrid();
		if (gr==null) return objectives;
		locs = gr.getOccupiedLocations();
		for (Location loc : locs) 
		{
			if (gr.get(loc) instanceof Bactor) 
		...
		for (Bactor b : bactors) 
		{
			objectives.add(new Objective(this.bactor, b));
		}
		return objectives;
	}
	public ArrayList<Objective> getObjectiveListBoth()
	{
		ArrayList<Objective> objectives = new ArrayList<Objective>();
		ArrayList<Bactor> bactors = new ArrayList<Bactor>();
	...
	private ArrayList<Objective> gatherObjectiveListBonus() {
		ArrayList<Objective> objectives= new ArrayList<Objective>();
		ArrayList<BonusActor> bonusActors = new ArrayList<BonusActor>();
		ArrayList<Location> locs = new ArrayList<Location>();
		Grid<Actor> gr = this.bactor.getGrid();
	...
	public ArrayList<Objective> getAdjacentObjectivesBugsFacingMeList()
	{
		ArrayList<Objective> getList = new ArrayList<Objective>();
		ArrayList<Actor> actors = this.bactor.getGrid().getNeighbors(this.bactor.getLocation());
		for (Actor a:actors)
		{
			if (a instanceof Bactor) 
			{
				...
				if (this.bactorFacingMe(b)==true) 
				{
					getList.add(new Objective(this.bactor,b));
				}
			}
		}
		return getList;
	...
	public boolean isObjectiveBugAdjacentClockwise() {
		int checkDirection=this.bactor.getDirection()+45;
		Grid<Actor> gr=this.bactor.getGrid();
		if (gr==null) return false;
		Location nextLoc=this.bactor.getLocation().getAdjacentLocation(checkDirection);
	...
	public boolean isObjectiveBugAdjacentCounterClockwise() {
		int checkDirection=this.bactor.getDirection()-45;
		Grid<Actor> gr=this.bactor.getGrid();
		if (gr==null) return false;
		Location nextLoc=this.bactor.getLocation().getAdjacentLocation(checkDirection);
	...
	public ArrayList<Objective> getAdjacentObjectiveBugsList()
	{
		ArrayList<Objective> getList = new ArrayList<Objective>();
		ArrayList<Actor> actors = this.bactor.getGrid().getNeighbors(this.bactor.getLocation());
		for (Actor a:actors)
	...
	public ArrayList<Objective> getSortedObjectiveBugsList(int order, Attribute attribute)
	{
		ArrayList<Objective> rawTargetList = this.getObjectiveListBugs();
		if (rawTargetList.size() == 0)
		{
			return rawTargetList;
		}
			...
			if(order == Informer.SORT_ASCENDING)
			{
				for (int i = 0; i < rawTargetList.size(); i++)
				{
					for (int j = i+1; j < rawTargetList.size(); j++)
					{
						if (field(attribute, rawTargetList.get(i)) > field(attribute, rawTargetList.get(j)))
						{
							Objective temp = rawTargetList.get(i);
							rawTargetList.set(i, rawTargetList.get(j));
							rawTargetList.set(j, temp);
						}
					}
	...
	public ArrayList<Objective> getSortedObjectiveBothList(int order, Attribute attribute)
	{
		ArrayList<Objective> rawTargetList = this.getObjectiveListBoth();
		if (rawTargetList.size() == 0)
		{
			return rawTargetList;
		}
	...
	public ArrayList<Objective> getObjectiveBonusList()
	{
		ArrayList<Objective> bonuses=this.gatherObjectiveListBonus();
		this.sortObjectiveByStepDistance(bonuses);
		return bonuses;
	}
	...
	private ArrayList<Objective> gatherObjectiveBonusByType(Bonus type) {
		ArrayList<Objective> rawList = this.getObjectiveBonusList();
		ArrayList<Objective> sortedList = new ArrayList<Objective>();
		for (Objective b: rawList)
		{
			if (b.getEffect().equals(type))
			{
				sortedList.add(b);
	...
	public ArrayList<Objective> getObjectiveBonusByType(Bonus type)
	{
		ArrayList<Objective> bonuses = gatherObjectiveBonusByType(type);		
		this.sortObjectiveByStepDistance(bonuses);
		return bonuses;
	}
	...
	private void sortObjectiveByStepDistance(ArrayList<Objective> bonuses) {
		if (bonuses==null) return;
		if (bonuses.size()>1) {
			for (int j=0; j<bonuses.size(); j++) {
				for (int i=0; i<bonuses.size()-1; i++) {
					if (bonuses.get(i).getStepsDistance()>bonuses.get(i+1).getStepsDistance()) {
						Objective temp=bonuses.get(i);
						bonuses.set(i,bonuses.get(i+1));
						bonuses.set(i+1, temp);
					}
				}
	...
	private void sortObjectiveByPathDistance(ArrayList<Objective> bonuses) {
		if (bonuses==null) return;
		if (bonuses.size()>1) {
			for (int j=0; j<bonuses.size(); j++) {
				for (int i=0; i<bonuses.size()-1; i++) {
	...
	public Objective ObjectiveBugFoeInFront()
	{
		Objective targetFoeInFront=null;
		if (this.bactor.foeInFront()==true) 
		{
			Grid<Actor> gr=this.bactor.getGrid();
			if (gr!=null)
			{
				...
				if (myLoc!=null) 
				{
					...
					if (gr.isValid(nextLoc))
					{
						...
						{
							Bactor opponent=(Bactor)gr.get(nextLoc);
							targetFoeInFront=new Objective(this.bactor, opponent);
						}
					}
				}
			}
	...
	public Objective ObjectiveBonusInFront()
	{
		Objective bonusInFront=null;
		if (this.bactor.bonusInFront()==true) 
		{
			Grid<Actor> gr=this.bactor.getGrid();
			if (gr!=null)
			{
				...
				if (myLoc!=null) 
				{
					...
					if (gr.isValid(nextLoc))
					{
						...
						{
							BonusActor bonusActor=(BonusActor)gr.get(nextLoc);
							bonusInFront=new Objective(this.bactor, bonusActor);
						}
					}
				}
			}
	...
	public Objective ObjectiveInFront()
	{
		Objective ObjectiveInFront=null;
		if (this.bactor.bonusInFront()==true) 
		{
			Grid<Actor> gr=this.bactor.getGrid();
			if (gr!=null)
			{
				...
				if (myLoc!=null) 
				{
					...
					if (gr.isValid(nextLoc))
					{
						...
						{
							BonusActor bonusActor=(BonusActor)gr.get(nextLoc);
							ObjectiveInFront=new Objective(this.bactor, bonusActor);
						}
						if (gr.get(nextLoc) instanceof Bactor) 
						{
							Bactor opponent=(Bactor)gr.get(nextLoc);
							ObjectiveInFront=new Objective(this.bactor, opponent);
						}
					}
				}
			}
		}
		return ObjectiveInFront;
	}
	/**
	...
	public Bonus getObjectiveEffect(Objective t)
	{
		return t.getCurrentEffect();
	}
	/**
	...
	public int field(Attribute attribute, Objective t) 
	{
		int field=0;
		if (t!=null)
		{
	...
	public Path getPathToObjective(Objective target) {
		if (target!=null && target.targetValid()&& target.getTargetName()!=null) {
			return new Path(this.bactor.getLocation(), this.bactor.getDirection(), target.getBactor().getLocation());
		} else if(target!=null && target.targetValid()&& target.getTargetName()==null) {
			return new Path(this.bactor.getLocation(),this.bactor.getDirection(), new Location(target.getRow(),target.getCol()));
		}else{
			return new Path();
		}
	}
	...
	public ArrayList<Objective> getObjectiveBonusList(DistanceMeasure distanceMeasure) {
		ArrayList<Objective> bonuses=this.gatherObjectiveListBonus();
		if (distanceMeasure.equals(DistanceMeasure.STEP)) {
			this.sortObjectiveByStepDistance(bonuses);
		} else if (distanceMeasure.equals(DistanceMeasure.PATH)) {
			this.sortObjectiveByPathDistance(bonuses);
		}
		return bonuses;
	}
	/**
	...
	public ArrayList<Objective> getObjectiveBonusByType(ArrayList<Bonus> bonusTypes, DistanceMeasure distanceMeasure) {
		ArrayList<Objective> bonuses=new ArrayList<Objective>();
		for (Bonus type:bonusTypes) {
			bonuses.addAll(this.gatherObjectiveBonusByType(type));
		}
		if (distanceMeasure.equals(DistanceMeasure.STEP)) {
			this.sortObjectiveByStepDistance(bonuses);
		} else if (distanceMeasure.equals(DistanceMeasure.PATH)) {
	...
	public ArrayList<Objective> getObjectiveBonusByType(Bonus type, DistanceMeasure distanceMeasure) {
		ArrayList<Objective> bonuses=this.gatherObjectiveBonusByType(type);
		if (distanceMeasure.equals(DistanceMeasure.STEP)) {
			this.sortObjectiveByStepDistance(bonuses);
		} else if (distanceMeasure.equals(DistanceMeasure.PATH)) {
			this.sortObjectiveByPathDistance(bonuses);
	...
	public ArrayList<Objective> getObjectiveBothByPathDistance() {
		ArrayList<Objective> targets=new ArrayList<Objective>();
		targets.addAll(this.getObjectiveListBoth());
		this.sortObjectiveByPathDistance(targets);
		return targets;
	}
	public ArrayList<Objective> getObjectiveBugsByPathDistance() {
		ArrayList<Objective> targets=new ArrayList<Objective>();
		targets.addAll(this.getObjectiveListBugs());
		this.sortObjectiveByPathDistance(targets);
		return targets;
	}
	public ArrayList<Objective> getObjectiveBonusByPathDistance() {
		ArrayList<Objective> targets=new ArrayList<Objective>();
		targets.addAll(this.getObjectiveBonusList());
		this.sortObjectiveByPathDistance(targets);
		return targets;
	}
	/**

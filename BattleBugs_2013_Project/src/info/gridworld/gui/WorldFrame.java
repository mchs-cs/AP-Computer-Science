...
import battle.pack.BattleBugsPrefs;
import battle.pack.BattleInfoDialog;
import battle.pack.BattleRunner;
import battle.pack.BattleWorld;
import battle.pack.Reporter;
import battle.pack.WindowPrefs;
/**
...
public class WorldFrame<T> extends JFrame
{
	...
	private WindowPrefs myPrefs;
	
	/**
	 * Constructs a WorldFrame that displays the occupants of a world
	 * @param setWorld the world to display
	 */
	public WorldFrame(World<T> setWorld)
	{
		this.world = setWorld;
		count++;
		...
		if(!BattleRunner.FAST_RUN){
					...
					if (count == 0) {
						if (world!=null) {
							if (world instanceof BattleWorld) {
								BattleWorld bw=(BattleWorld)world;
								Reporter r=bw.getDoBattle().getReporter();
								if (r!=null) {
			...
			for (String name : setWorld.getGridClasses())
				try
			{
			...
			display.setGrid(gr);
			getPrefs();
			this.setLocation(myPrefs.winX,myPrefs.winY);
			this.setSize(myPrefs.winW,myPrefs.winH);

		} else control = new GUIController<T>(this, display, displayMap, resources);
	}
	...
	public void displaySetGrid(Grid<T> setGrid) {
		if (BattleRunner.FAST_RUN) return;
		display.setGrid(setGrid);
		repaint(); 	
	}
	/**
	...
	private void getPrefs() {
		myPrefs=BattleInfoDialog.battleBugsPrefs.getBattleBugsWindowPrefs();
		if (myPrefs==null) myPrefs=new WindowPrefs(this, null); 
//		try {
//			FileInputStream fin = new FileInputStream("bbWin_"+bbPrefs.getPrefsName());
//			ObjectInputStream ois = new ObjectInputStream(fin);
//			myPrefs = (WindowPrefs) ois.readObject();
//			ois.close();
//		} catch(Exception ex) {
//			myPrefs=new WindowPrefs(this, null);
//		}
	}
	private void storePrefs() {
		myPrefs=new WindowPrefs(this, null);
		BattleInfoDialog.battleBugsPrefs.setBattleBugsWindowPrefs(myPrefs);
//		try {
//			FileOutputStream fout = new FileOutputStream("bbWin_"+bbPrefs.getPrefsName());
